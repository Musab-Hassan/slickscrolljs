...
class slickScroll {
    ...
    // default properties
    private defaults: { root: string, duration: number, easing: string, offsets: { speedX: number, speedY: number } } = {
        root: "body",
        duration: 1000,
    ...
    public momentumScroll(dataObj: momentumScrollStruct) {
        ...
        dataObj = (<any>Object).assign({}, this.defaults, dataObj); // assign defaults to parameter object if not assigned
        
        let pl: {x: number, y: number}, startStamp: number;
        let rootElem = <any> document.querySelector(dataObj.root);
        ...
        if (checkCompatiblity()) {
            rootElem.addEventListener("scroll", (event: any) => {
                ...
                if (this.offsets[dataObj.root]) {
                    ...
                    this.offsets[dataObj.root].forEach((e: any) => {
                        ...
                        let elements: NodeListOf<any> = document.querySelectorAll(e.element);
            ...
            if (this.fixed[dataObj.root]) {
                this.fixed[dataObj.root].forEach((e: any) => {
                    let elements: NodeListOf<any> = document.querySelectorAll(e);
                    for (e of elements as any) {
            ...
            easeFrames(tl, pl, startStamp, (position: {x: number, y: number}) => {
                ...
                if (THIS.offsets[dataObj.root]) {
                    ...
                    THIS.offsets[dataObj.root].forEach((e: any) => {
                        ...
                        let elements: any = document.querySelectorAll(e.element);
        ...
        function addOffset(obj: {element: string, speedY: number, speedX: number}) {
            if (typeof obj !== "object") return;
            if (!("element" in obj)) return;
            // Check if offset is already set
            const find = THIS.offsets[dataObj.root].find((i: any) => i.element == obj.element);
        ...
        function addFixedOffset(e: string) {
            if (!document.querySelector(e) || THIS.fixed[dataObj.root].includes(e)) return;
            THIS.fixed[dataObj.root].push(e);
        }

        // Allows offsets to be destroyed
        function removeOffset(element: string) {
            THIS.fixed[dataObj.root] = THIS.fixed[dataObj.root].filter((e: any) => e != element);
            THIS.offsets[dataObj.root] = THIS.offsets[dataObj.root].filter((e: any) => e.element != element);

            let elements: NodeListOf<any> = document.querySelectorAll(element)
